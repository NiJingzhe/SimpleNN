...
class Model:
    ...
    def _compute_metrics(self, y_pred: np.ndarray, y_true: np.ndarray) -> Dict[str, float]:
        """计算评估指标
    ...
    def fit(self, 
        ...
        for epoch in range(epochs):
            ...
            for i in range(iterations_per_epoch):
                ...
                try:
                    ...
                    if self.metrics:
                        ...
                        if len(batch_pred.shape) > 1 and batch_pred.shape[1] == 1 and len(batch_y.shape) == 1:
                            batch_y_reshaped = batch_y.reshape(-1, 1)
                        else:
                            batch_y_reshaped = batch_y
                        batch_metrics = self._compute_metrics(batch_pred, batch_y_reshaped)
                        for metric_name, value in batch_metrics.items():
                            epoch_metrics[metric_name] += value * (end_idx - start_idx)
                    
                    # 显示进度
                    if verbose == 2:
                        print(f"Epoch {epoch+1}/{epochs} - Batch {i+1}/{iterations_per_epoch} - Loss: {batch_loss:.4f}")
                    elif verbose == 1 and (i + 1) % max(1, iterations_per_epoch // 10) == 0:
                        progress = (i + 1) / iterations_per_epoch
                        bar_length = 30
                        bar = '=' * int(bar_length * progress) + '>' + ' ' * (bar_length - int(bar_length * progress) - 1)
                        print(f"\rEpoch {epoch+1}/{epochs} - [{bar}] {progress*100:.1f}% - Loss: {batch_loss:.4f}", end='')
                except Exception as e:
            ...
            if verbose > 0:
                ...
                val_str = f" - val_loss: {val_loss:.4f}" if validation_data is not None else ""
                for metric_name, value in val_metrics.items():
                    val_str += f" - val_{metric_name}: {value:.4f}"
                
                print(f"\rEpoch {epoch+1}/{epochs} - {epoch_time:.2f}s - loss: {epoch_loss:.4f}{metrics_str}{val_str}")
        ...
        if metrics:
            for metric in metrics:
                if isinstance(metric, Metric):
                    ...
                    raise ValueError(f"指标必须是字符串或Metric对象，而不是{type(metric)}")
